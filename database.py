...
def init_db():
    ...
    # 1. TABLA DE USUARIOS (Login)import pymysql
import os
...
# --- SISTEMA DE GESTIÓN "BEAUTY TEAM SALON" ---
# Inicializa la base de datos MySQL con tablas y datos de prueba.
...
def init_db():
    ...
    if cursor.fetchone()['cnt'] == 0:
        ...
        appointments = [
            (1, 1, 1, today, '10:00:00', 'Pendiente'),
            (2, 2, 2, today, '11:00:00', 'Asistida'),
            (3, 3, 5, today, '12:30:00', 'Pendiente'),
        ]
        cursor.executemany(
            "INSERT INTO appointments (client_id, employee_id, service_id, date, time, status) VALUES (%s, %s, %s, %s, %s, %s)",
            appointments
        )
    ...
    if cursor.fetchone()['cnt'] == 0:
        sales = [
            (1, 85.00, 'Efectivo', 'admin@salon.com'),
            (2, 45.00, 'Transferencia', 'admin@salon.com'),
            (None, 11.50, 'Efectivo', 'admin@salon.com'),
        ]
        cursor.executemany(
    ...
    cursor.close()
    conn.close()
    print("Base de datos MySQL inicializada con éxito.")
